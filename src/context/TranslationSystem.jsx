...
const translateWithRetry = async (text, targetLanguage, maxRetries) => {
  ...
  while (retries < maxRetries) {
    try {
      // Create the request to Google's Gemini AI
      const prompt = `
  ...
  while (retries < maxRetries) {
    try {
      ...
      if (!Array.isArray(texts) || texts.length === 0) {
        return "Invalid input: texts must be a non-empty array";
      }
    ...
    `;
      ...
      let translatedTexts;
      try {
        translatedTexts = JSON.parse(response);

        // Ensure we got back an array of the right length
        if (
        ...
        `;
            ...
            return singleResult.response.text().trim();
          })
        );
      }
